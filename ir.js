(function ($) {
    $.fn.iranPlaces = function (settings) {
        let provinces = [
            'تهران',
            'خراسان رضوی',
            'اصفهان',
        ...
        if (settings.provinceTarget) {
            provinces.forEach(val => {
                $(settings.provinceTarget).append(`<option value="${val}">${val}</option>`);
            });
        }

        if (settings.defaultProvince) {
            $(settings.provinceTarget).val(settings.defaultProvince);
            cities.forEach(val => {
                val == settings.defaultProvince ? $(settings.cityTarget).empty().html(cities.cityNames) : ''
            });
        }


        $(document).on('change', settings.provinceTarget, function () {

          if ($(this).val() === 'آذربایجان غربی') {
                $(settings.cityTarget).empty().html(`
